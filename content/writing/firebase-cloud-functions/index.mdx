...
From a frontend engineer perspective, your skillset is limited when you need some backend developement and/or server operations. Serverless techlonoligies opens that field for you.
...
Let us use this "helloWorld" example. Notice that I disabled the <a href='https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS' target='_blank' rel='nofollow noopener noreferrer'>`CORS` header</a> since we are in developement.
...
For the `url` variable, we should modify it to accomodate for the environment (developement vs production):
...
There is still much we can do here. We will explore other thing in the next posts. But, if you are in a hurry, you can check the <a href='https://firebase.google.com/docs/functions' target='_blank' rel='nofollow noopener noreferrer'>the Firebase Functions docs</a>.
